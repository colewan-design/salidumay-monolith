    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(isLoggedIn)) {
        ...
        if (unref(user)?.avatar) {
          ...
          _push(`<span data-v-bac266f9>${ssrInterpolate(unref(user)?.name?.[0]?.toUpperCase() || "U")}</span>`);
        }
        _push(`</div><div class="user-info" data-v-bac266f9><span class="user-name" data-v-bac266f9>${ssrInterpolate(unref(user)?.name)}</span><span class="user-email" data-v-bac266f9>${ssrInterpolate(unref(user)?.email)}</span></div></div><button class="signout-btn" data-v-bac266f9>Sign Out</button><!--]-->`);
      } else {
        _push(`<!--[-->`);
        _push(ssrRenderComponent(unref(RouterLink), {
          to: "/login",
...
//# sourceMappingURL=default-v07NTEjJ.js.map
