...
import {
  getPopularSeries, getTopRatedSeries, getTrendingSeries,
  getAiringTodaySeries, getTvGenres, getSeriesByGenre, searchSeries,
} from '~/services/tvSeries.js'
...
onMounted(async () => {
  genres.value = await getTvGenres().catch(() => [])
  load(1)
})
</script>
  ...
  <div class="page">
    ...
    <section class="banner">
      ...
      <div class="banner-content">
        ...
        <h1 class="title">TV <span class="accent">Series</span></h1>
        <p class="sub">Drama · Thriller · Sci-Fi · Fantasy · Comedy</p>
        <div class="search-bar">
          <svg class="search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2">
            <circle cx="11" cy="11" r="7"/><path d="m21 21-4.35-4.35"/>
          </svg>
          <input v-model="searchQuery" class="search-input" placeholder="Search series…" autocomplete="off" />
    ...
    <main class="wrap">
      ...
      <div v-else-if="items.length" class="grid">
        <article v-for="s in items" :key="s.id" class="card" @click="navigateTo(`/series/${s.id}`)">
          <div class="card-img">
            <img :src="s.image" :alt="s.title" loading="lazy" />
            <span class="series-badge">SERIES</span>
            <div v-if="s.number_of_seasons" class="seasons-badge">{{ s.number_of_seasons }}S</div>
          ...
          <div class="card-body">
            ...
            <div class="meta">
              ...
              <span class="rating">★ {{ typeof s.rating === 'number' ? s.rating.toFixed(1) : s.rating }}</span>
            </div>
          </div>
        </article>
      </div>
...
.banner-bg {
  ...
  background:
    radial-gradient(ellipse at 30% 50%, rgba(0,240,255,0.1) 0%, transparent 55%),
    radial-gradient(ellipse at 75% 30%, rgba(255,45,120,0.07) 0%, transparent 55%),
    var(--bg);
}
.banner-content { position: relative; z-index: 2; text-align: center; padding: 2rem; }
.tag  { font-size: .7rem; font-weight: 800; letter-spacing: .2em; text-transform: uppercase; color: var(--cyan); }
...
.search-bar:focus-within { border-color: rgba(0,240,255,0.5); }
.search-icon { width: 1rem; height: 1rem; color: var(--text-muted); flex-shrink: 0; margin-right: .6rem; }
.search-input { flex: 1; background: none; border: none; outline: none; color: var(--text); font-size: .9rem; }
.search-input::placeholder { color: var(--text-muted); }
.search-spin { width: 14px; height: 14px; border: 2px solid rgba(255,255,255,.15); border-top-color: var(--cyan); border-radius: 50%; animation: spin .8s linear infinite; flex-shrink: 0; margin-left: .5rem; }
...
.tab.active { background: var(--cyan); border-color: var(--cyan); color: #060e14; }
...
.card:hover { transform: translateY(-6px) scale(1.02); box-shadow: 0 0 0 1px rgba(0,240,255,.35), 0 8px 40px rgba(0,240,255,.1); }
...
.series-badge { position: absolute; top: 8px; left: 8px; background: rgba(0,240,255,.2); color: var(--cyan); border: 1px solid rgba(0,240,255,.4); font-size: .58rem; font-weight: 800; padding: .18rem .4rem; border-radius: 4px; z-index: 2; letter-spacing: .08em; }
.seasons-badge { position: absolute; top: 8px; right: 8px; background: rgba(0,0,0,.7); color: rgba(255,255,255,.8); font-size: .58rem; font-weight: 800; padding: .18rem .4rem; border-radius: 4px; z-index: 2; }
...
.play-btn { width: 44px; height: 44px; border-radius: 50%; background: var(--cyan); display: flex; align-items: center; justify-content: center; box-shadow: 0 0 20px rgba(0,240,255,.5); }
.play-btn svg { width: 18px; height: 18px; color: #060e14; }
...
.load-more-btn { padding: .7rem 2.5rem; background: linear-gradient(135deg, var(--cyan), #00b8dc); color: #060e14; border: none; border-radius: 6px; font-size: .88rem; font-weight: 800; cursor: pointer; transition: box-shadow .2s; display: inline-flex; align-items: center; gap: .5rem; }
